...
class TooLongMessagesTestCase(TestCase):
    ...
    def test_too_long_message_template(self):
        ...
        (
            ...
            .check(
                S.val().delete_message(chat_id=1, message_id=123),
                S.val().send_message(chat_id=1, text='X1 (1)/1'),
                Return
            )
            .validate()
        )
    ...
    def test_long_messages_delay(self):
        ...
        (
            ...
            .check(
                S.val().delete_message(chat_id=1, message_id=101),
                S.val().edit_message_text(text='X1 (1)/2', chat_id=1, message_id=10001),
                Return
            )
            .send(self.msg(102,11,2))
            .check(
                S.val().delete_message(chat_id=1, message_id=102),
                S.val().edit_message_text(text='X1 (1)/2,X2 (2)/1', chat_id=1, message_id=10001),
                Return
            )
            ...
            .check(
                ...
                S.val().edit_message_text(text='X1 (1)/2', chat_id=1, message_id=10002),
                Return
            )
            .validate()
        )
