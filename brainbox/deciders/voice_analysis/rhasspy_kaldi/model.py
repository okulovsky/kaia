...
class RhasspyKaldiModel(DownloadableModel):
    ...
    def download(cls, models: list['DownloadableModel'], controller: IController):
        ...
        for model in models:
            ...
            controller.context.api_callback.log(f"Downloading models for language {model.language}: running endpoint")
            shutil.copy(
            ...
            try:
                ...
                controller.wait_for_boot(port)
                controller.context.api_callback.log("Endpoint booted, requesting download")
                requests.post(f'http://127.0.0.1:{port}/api/download-profile')
                controller.stop(instance_id)
            finally:
                controller.context.instance_registry.release_port(port)
            ...
            controller.context.api_callback.log("All good")
