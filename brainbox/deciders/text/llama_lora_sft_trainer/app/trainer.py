...
from interface import TrainingSettings
...
class Trainer:
    def __init__(self, model_id:str, adapter_name: str, guid: str) -> None:
        ...
        self.guid = guid
        self.exp_folder = Path("/resources/experiments") / self.model_id / self.adapter_name / self.guid
    ...
    def _load_dataset(self):
        ...
        dataset = load_dataset(
            "json",
            data_files={
                "train": str(Path("/resources/experiments") / self.model_id / self.adapter_name / self.guid / "train.jsonl"),
            },
        )
...
if __name__ == '__main__':
    import sys
    _model_id, _adapter_name, _guid = sys.argv[1], sys.argv[2], sys.argv[3]
    _trainer = Trainer(_model_id, _adapter_name, _guid)
    _trainer.train()
    _trainer.convert_lora_to_gguf()
