...
from brainbox.framework.app.jobs import JobsApi, JobSummary
...
class TestGetInfo(TestCase):
    def test_returns_info(self):
        ...
        with Session(engine) as s:
            ...
            s.commit()
        with JobsApi.test(engine) as api:
            self.assertEqual({'source': 'mic', 'lang': 'en'}, api.get_job('j1').info)
    ...
    def test_none_info(self):
        ...
        with Session(engine) as s:
            ...
            s.commit()
        with JobsApi.test(engine) as api:
            self.assertIsNone(api.get_job('j1').info)

    def test_unknown_id_raises(self):
        engine = _engine()
        with JobsApi.test(engine) as api:
            with self.assertRaises(Exception):
                api.get_job('no-such-id').info
...
class TestGetError(TestCase):
    def test_returns_error_message(self):
        ...
        with Session(engine) as s:
            ...
            s.commit()
        with JobsApi.test(engine) as api:
            self.assertEqual('something went wrong', api.get_job('j1').error)
    ...
    def test_none_when_no_error(self):
        ...
        with Session(engine) as s:
            ...
            s.commit()
        with JobsApi.test(engine) as api:
            self.assertIsNone(api.get_job('j1').error)

    def test_unknown_id_raises(self):
        engine = _engine()
        with JobsApi.test(engine) as api:
            with self.assertRaises(Exception):
                api.get_job('no-such-id').error
...
class TestGetLog(TestCase):
    def test_returns_log_entries(self):
        ...
        with Session(engine) as s:
            ...
            s.commit()
        with JobsApi.test(engine) as api:
            self.assertEqual(['step 1', 'step 2', 'done'], api.get_job('j1').log)
    ...
    def test_none_when_no_log(self):
        ...
        with Session(engine) as s:
            ...
            s.commit()
        with JobsApi.test(engine) as api:
            self.assertIsNone(api.get_job('j1').log)

    def test_unknown_id_raises(self):
        engine = _engine()
        with JobsApi.test(engine) as api:
            with self.assertRaises(Exception):
                api.get_job('no-such-id').log
