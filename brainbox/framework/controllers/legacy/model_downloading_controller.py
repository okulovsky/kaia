...
from ..architecture import IController
...
class IModelDownloadingController:
    ...
    def download_models(self, models: Any):
        ...
        models_to_download = []
        for model in models:
            controller.context.api_callback.log(f"Checking model {model}")
            location = model.get_local_location(self)
            if location.is_dir() or location.is_file():
                controller.context.api_callback.log("Model is already downloaded")
                continue
            controller.context.api_callback.log("Model needs to be downloaded")
            models_to_download.append(model)
        if len(models_to_download) == 0:
            controller.context.api_callback.log("All models are downloaded")
        else:
            controller.context.api_callback.log(f"Downloading {len(models_to_download)} models")
            _type = self.get_downloadable_model_type()
