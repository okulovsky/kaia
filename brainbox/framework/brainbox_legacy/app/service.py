...
from sqlalchemy import create_engine, select, Engine, func, Float, case
...
class BrainBoxService(IBrainboxService):
    ...
    def run(self):
        ...
        self.loop = MainLoop(core, self.settings.planner, core.command_queue, self.settings.stop_controllers_at_termination)
        self.loop_thread = Thread(target=self.loop.run, daemon=True)
            ...
            for job in jobs:
                ...
                if not job.success:
                    if allow_failures:
                        ...
                        stars='*~'*30
                        raise ValueError(f"{job.decider}/{job.parameter}:{job.method} threw an error:\n{stars}\n{job.error}\n{stars}")
                else:
    ...
    def result(self, id: str):
        with Session(self.engine) as session:
            result = list(session.scalars(select(Job.result).where(Job.id == id)))
            if len(result)==0:
                return None
            return result[0]
    ...
    def summary(self, ids: list[str]|None = None, batch_id: str|None = None) -> list[dict]:
        query = select(
            Job.id, Job.decider, Job.parameter, Job.method, Job.batch, Job.info, Job.progress,
            Job.received_timestamp,
            Job.ready_timestamp.is_not(None).label('ready'), Job.ready_timestamp,
            Job.assigned_timestamp.is_not(None).label('assigned'), Job.assigned_timestamp,
            Job.accepted_timestamp.is_not(None).label('accepted'), Job.accepted_timestamp,
            Job.finished_timestamp.is_not(None).label('finished'), Job.finished_timestamp,
            Job.responding_timestamp,
    ...
    def job(self, id: str):
        with Session(self.engine) as session:
            result = list(session.scalars(select(Job).where(Job.id == id)))
        if len(result) == 0:
    ...
    def batch_progress(self, batch: str):
        with Session(self.engine) as session:
            stmt = (
                select(func.avg(case((Job.finished_timestamp.is_not(None), 1), else_=0).cast(Float)))
