...
from .endpoint_decorator import ENDPOINT_ATTR

SERVICE_ATTR = '_service_attributes'
...
def service(cls=None):
    def decorator(c):
        ...
        setattr(c, SERVICE_ATTR, ServiceAttributes(n))
        return c
    if cls is not None:
...
class ServiceModel:
    ...
    def parse(value: Any) -> 'ServiceModel':
        ...
        service_entries = [entry for entry in desc.mro if SERVICE_ATTR in entry.type.__dict__]
        if not service_entries:
            raise TypeError(
                f"Type '{desc.mro[0].type.__name__}' and none of its bases are decorated with @service"
            )

        attrs: ServiceAttributes = getattr(service_entries[0].type, SERVICE_ATTR)
        ...
        for entry in service_entries:
            ...
            for name, member in vars(actual_cls).items():
                ...
                if not callable(func):
                    continue
                if not hasattr(func, ENDPOINT_ATTR):
                    continue
                # Always parse the interface member (which carries ENDPOINT_ATTR and the
                # correct signature).  When value is a service *instance*, getattr(value, name)
                # returns the concrete override which has no ENDPOINT_ATTR, so EndpointModel.parse
