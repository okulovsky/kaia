...
from foundation_kaia.marshalling_2.marshalling.model import CallModel, endpoint, parse_json
...
class TestJsonBody(unittest.TestCase):
    def helper(self, f, *args):
        ...
        parsed = {}
        parse_json(parsed, c.json, call_model.endpoint_model.params.json_params)
        return c, parsed
    ...
    def test_dataclass_serialized_to_json(self):
        ...
        self.assertEqual({'p': {'value': 1}}, c.json)
    ...
    def test_multiple_json_params(self):
        ...
        self.assertEqual({'p': {'value': 2}, 'q': {'value': 3}}, c.json)
        self.assertEqual("/func", c.url)
    ...
    def test_primitive_path_plus_json_body(self):
        ...
        self.assertEqual({'p': {'value': 99}}, c.json)
    ...
    def test_force_json_params_primitive(self):
        ...
        self.assertEqual({'a': 5, 'b': 'hello'}, c.json)
        self.assertEqual('/func', c.url)
        self.assertEqual({'a': 5, 'b': 'hello'}, kw)

    def test_force_json_params_nullable(self):
        # Without force_json_params, int|None goes to query string; with it, goes to json body.
        @endpoint(force_json_params=True, verify_abstract=False)
        def func(a: int, b: int | None) -> None:
            pass
        c, kw = self.helper(func, 1, None)
        self.assertEqual({'a': 1, 'b': None}, c.json)
        self.assertEqual('/func', c.url)
    ...
    def test_json_parameter_default(self):
        ...
        self.assertEqual({'p': {'value': 1}, 'q': {'value': 2}}, c.json)
        self.assertEqual("/func", c.url)
