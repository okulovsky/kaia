from ....daemon import StatefulRecorderState


class StatesHandler:
    def __init__(self):
        self.shapes = []
        self.annotations = []
        self.last_state: StatefulRecorderState|None = None
        self.last_relative_time: float|None = None
        self.stat_to_color = {
            StatefulRecorderState.Standby: "Honeydew",
            StatefulRecorderState.Open: "LightSteelBlue",
            StatefulRecorderState.Record: "MistyRose",
        }



    def add(self, relative_time: float, item: StatefulRecorderState):
        if self.last_state is not None and self.last_state in self.stat_to_color:
            self.shapes.append(dict(
                type="rect",
                xref="x",
                yref="paper",
                x0=self.last_relative_time,
                y0=0,
                x1=relative_time,
                y1=1,
                fillcolor=self.stat_to_color[self.last_state],
                opacity=0.5,
                line_width=0,
            ))

            self.annotations.append(dict(
                x=self.last_relative_time,
                y=1,  # верхний левый угол прямоугольника
                text=self.last_state.name,
                textangle=-90,
                xref="x", yref="paper",
                showarrow=False,
                font=dict(size=12, color="black"),
                xanchor="left",
                yanchor="top"
            ))

        self.last_relative_time = relative_time
        self.last_state = item


    def get_shapes_and_annotations(self) -> tuple[list, list]:
        self.add(0, StatefulRecorderState.Standby)
        return self.shapes, self.annotations
