...
from ..state import InitialStateFactory, State, WorldFields
import os
...
class AvatarSettings:
    initial_state_factory: InitialStateFactory = field(default_factory=lambda: InitialStateFactory.Simple({
        WorldFields.character: 'character_0'
    }))
    port: int = 8092
...
class AvatarService(IAvatarApi):
    ...
    @endpoint(url='/dub/paraphrase/set_replies', method='POST')
    def dub_paraphrase_set_replies(self, replies: tuple[Template,...]):
        self.state.initialize_replies(replies)

    @endpoint(url='/dub/paraphrase/get_replies', method='GET')
    def dub_paraphrase_get_replies(self) -> tuple[Template,...]:
        return self.state.replies
    ...
    def recognition_train(self, intents: tuple[IntentsPack,...]):
        state = self.state
        state.initialize_intents(intents)
