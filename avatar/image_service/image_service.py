...
class ImageService:
    ...
    def mlm_record_to_file(self, content: MediaLibrary.Record):
        image = File(
            content.filename,
            content.get_content(),
            File.Kind.Image
        )
        image.metadata = ImageMetadata(content.tags)
        return image
    ...
    def get_new_image(self, state: State) -> File:
        ...
        state.add_memory(ImageMemoryItem(content.filename))
        return image
    ...
    def get_current_image_description(self, state: State) -> str|None:
        ...
        record = self.settings.manager.get_content_by_id(last_image.image_file_name)
