...
class TestClient(TestCase):
    ...
    def _msgs(self, result):
        return [e.message for e in result.messages]
    ...
    def test_push_and_pull_round_trip(self):
        ...
        self.assertIsInstance(result.messages[0].message, MsgA)
        self.assertEqual('hello', result.messages[0].message.value)
    ...
    def test_pull_after_max_messages_continues_from_correct_position(self):
        ...
        self.assertEqual(['0', '1'], [e.message.value for e in r1.messages])
        r2 = client.base_pull(timeout_in_seconds=0)
        self.assertEqual(['2', '3'], [e.message.value for e in r2.messages])
    ...
    def test_pull_unknown_type_becomes_generic_message(self):
        ...
        result = client.base_pull(timeout_in_seconds=0)
        self.assertEqual(1, len(result.messages))
        self.assertIsInstance(result.messages[0].message, GenericMessage)
        self.assertEqual('some.Unknown.Type', result.messages[0].message.content_type)
    ...
    def test_pull_preserves_envelop(self):
        ...
        client.push(msg)
        result = client.base_pull(timeout_in_seconds=0)
        self.assertEqual(msg.envelop.id, result.messages[0].message.envelop.id)
    ...
    def test_mixed_message_types_deserialized_correctly(self):
        ...
        result = client.base_pull(timeout_in_seconds=0)
        self.assertIsInstance(result.messages[0].message, MsgA)
        self.assertIsInstance(result.messages[1].message, MsgB)
        self.assertEqual('from_a', result.messages[0].message.value)
        self.assertEqual('from_b', result.messages[1].message.value)
    ...
    def test_element_session_matches_client_session(self):
        client = self._client()
        client.push(MsgA('hello'))
        result = client.base_pull(timeout_in_seconds=0)
        self.assertEqual('default', result.messages[0].session)
