import { WebcamHandlerSettings } from './webcamHandlerSettings'
...
export class WebcamHandlerSnapshot extends WebcamHandlerBase {
  ...
  private async _getPicture (target: HTMLCanvasElement): Promise<void> {
    ...
    try {
      ...
      const ctx = target.getContext('2d', { willReadFrequently: true } as any)!;
      (target.getContext('2d') as CanvasRenderingContext2D).drawImage(v, 0, 0, w, h)
    } finally {
  ...
  async takeNextPictureAndDetectMovement () {
    ...
    // Take new screenshot
    const currentContext = this._currentCanvas.getContext('2d') as CanvasRenderingContext2D
