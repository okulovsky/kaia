...
@dataclass
class FullRepoContainerBuilder(IContainerBuilder):
    name: str
    tag: str
    entry_point: Any
    ...
    def get_local_name(self) -> str:
        if self.custom_local_image_name is not None:
            return self.custom_local_image_name
        return f'{self._docker_conform_name(self.name)}:{self.tag}'
    ...
    def build_container(self) -> None:
        ...
        local_image_name = self.get_local_name()
        arguments = ['docker'] + self.build_command + ['--tag', local_image_name, str(docker_path)]
        ...
        if self.feed_input is None:
            ...
            if subprocess.run(arguments, input=self.feed_input.encode('ascii')).returncode !=0:
                raise ValueError(f'Docker call caused an error. Arguments\n{" ".join(arguments)}')
