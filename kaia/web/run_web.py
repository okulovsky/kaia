...
if __name__ == '__main__':
    data_path = Loc.data_folder/'web_page_only'
    if data_path.is_dir():
        shutil.rmtree(data_path)
    os.makedirs(data_path)
    with Loc.create_test_file() as db_path:
        aliases = MessagingComponent.create_aliases("avatar.messaging", "avatar.daemon", "kaia")
        settings = AvatarServerSettings(
            (
                ...
                MessagingComponent(db_path, aliases),
                FileCacheComponent(data_path),
            ),
            hide_logs=True,
