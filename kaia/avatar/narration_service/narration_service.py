...
from ..state import KnownFields, State
...
class NarrationService:
    ...
    def randomize_character(self, state: State) -> NarrationReply:
        next_character = self._random_change(state.get_state()[KnownFields.character], self.settings.characters)
        if next_character is None:
            return NarrationReply()
        change = {KnownFields.character: next_character}
        next_activity = self._random_change(state.get_state().get(KnownFields.activity, None), self.settings.activities)
        if next_activity is not None:
            change[KnownFields.activity] = next_activity
    ...
    def randomize_activity(self, state: State) -> NarrationReply:
        ...
        return self.apply_state_change(state, {KnownFields.activity: next_activity})

    def apply_state_change(self, state: State, change: dict[str, str]) -> NarrationReply:
        current = state.get_state()
        change_image = False
        image_changing_fields = {KnownFields.activity, KnownFields.character}
        add_greeting = False
