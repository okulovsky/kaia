from typing import *
from ..structures import DubBinding, UnionDub, SequenceDub, ConstantDub, SetDub, ToStrDub
...
class WalkerState(ABC):
    ...
    def on_to_str(self, binding: DubBinding, dub: ToStrDub):
        raise ValueError(f'Algorithm {self} cannot work with a template where ToStrDub is used ({dub})')
    ...
    def _walk_sequence(self: 'WalkerState', sequence: SequenceDub, step_index: int) -> Iterable['WalkerState']:
        ...
        for state in WalkerState._walk_sequence(self, sequence, step_index-1):
            ...
            elif isinstance(step.type, ToStrDub):
                yield from state.on_to_str(step, step.type)
            else:
                raise ValueError(f'Step {step.type} was neither UnionDub, SequenceDub, ConstantDub or SetDub')
