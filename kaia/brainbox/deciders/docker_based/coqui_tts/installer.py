...
from kaia.brainbox.deciders.api.coqui_tts import CoquiTTSApi

class CoquiTTSInstaller(DockerBasedInstaller):
    ...
    def install_model_endpoint(self, model_name, mode) -> DockerBasedInstallerEndpoint:
        return DockerBasedInstallerEndpoint(self, deployment.DockerRun(
            mount_folders={self.settings.resource_folder:'/data'},
            additional_arguments=['--rm'],
            propagate_gpu=self.settings.use_gpu,
            command_line_arguments=[model_name, mode]
        ))

    def install(self, skip_models_installation: bool = False):
        self.server_endpoint.kill()
        self.build()
        self.executor.create_empty_folder(self.settings.get_voice_path(None))
        self.executor.upload_file(Path(__file__).parent/'test_voice.wav', self.settings.get_voice_path('test_voice'))
        if not skip_models_installation:
            for model in self.settings.builtin_models_to_download:
                self.install_model_endpoint(model.model_name, model.mode).run()

    def create_api(self):
        return CoquiTTSApi(f'{self.settings.address}:{self.settings.port}')


    def self_test(self, tc: TestCase):
        self.server_endpoint.run()

        url = f'http://{self.settings.address}:{self.settings.port}/'

        result = requests.get(url)
        tc.assertEqual(200, result.status_code)

        for model in self.settings.builtin_models_to_download:
            model_info = self.create_api().load_model(model.model_name)
            if model.test_dub:
                voice = model_info['speakers'][0]
                content = self.create_api().dub(model_info, f"Hello world from {model}", voice)
                check_if_its_sound(content, tc)

            if model.test_voice_clone:
                content = self.create_api().voice_clone(model_info, f'Voice clone from {model}', 'test_voice')
                check_if_its_sound(content, tc)
...
RUN cat /TTS/kaia/main.py

RUN apt-get update

RUN apt-get install espeak -y

ENTRYPOINT ["python3","/TTS/kaia/main.py"]
'''
