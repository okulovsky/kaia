...
from ..small_classes.job import BrainBoxBase
from .index_helper import _select, _pretty_sec
...
def _to_html(df):
    ...
    for row in Query.df(df.reset_index()):
        html.append('<tr>')
        html.append(f'<td><a href="/batch_index/{row.batch}">{row.batch}</td>')
...
def build_batch_report(session):
    df = _get_merged_frame(*_get_frames(session))
    if df is None:
        html = ''
    else:
        html = _to_html(df)
    return f'<html><body><table border=1>{html}</table></body></html>'
