from kaia import SingleLineKaiaSkill, KaiaAssistant, ChatCommand
from kaia.assistant.exception_in_skill import ExceptionInSkillReplies
...
class AssistantTestCase(TestCase):
    def test_error_handling(self):
        ...
        (Scenario(create_assistant)
         ...
         .check(1)
         .send('B')
         .check(ChatCommand, ExceptionInSkillReplies.answer.utter())
         .send('A')
         .check(2)
         .validate()
        )