from chara import Chara, brainbox_training_pipeline, ICase, BrainBoxCasePipeline, CaseCollection, logger, brainbox_pipeline
...
def piper_training(
    ...
    checkpoints = [CheckpointCase(PiperTraining.Checkpoint(**c)) for c in checkpoint_data]
    cases = CaseCollection(checkpoints)

    pipe = BrainBoxCasePipeline(_case_to_export, 'local_path', result_to_file=True)
    cases = Chara.call(pipe.__call__, "downloading_checkpoint")(cases).raise_if_any_error()

    Chara.call(brainbox_pipeline, "training_cleanup")([PiperTraining.new_task().delete(checkpoints[0].checkpoint.training_id)])
    return cases