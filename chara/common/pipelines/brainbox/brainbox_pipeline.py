...
from .cache import BrainBoxCache, TCase, TOption, BrainBoxPipelineResultItem, BrainBoxPipelineResultOption
...
class BrainBoxPipeline(Generic[TCase, TOption]):
    def __init__(self,
    ...
    def _match_tasks(self,
                     ...
                     stats: BrainBoxStats,
                     ) -> Iterable[BrainBoxPipelineResultItem]:
        index_to_result = {}
        ...
        for index, case in enumerate(cases):
            stats.cases_total += 1
            merge = BrainBoxPipelineResultItem(case, task_objects[index])
    ...
    def _perform_merge(
            self,
            merge: BrainBoxPipelineResultItem,
            stats: BrainBoxStats,
    ):
        try:
        ...
        for brainbox_option in brainbox_options:
            ...
            option = BrainBoxPipelineResultOption(brainbox_option)
            try:
        ...
        def _():
            if self.options_as_files:
                for option in cache.read_options():
                    cache.files.write(CharaApis.brainbox_api.cache.read_file(option))
