from chara.common import JsonlCache, logger
...
class FileCacheTestCase(TestCase):
    def test_file_cache(self):
        with Loc.create_test_folder() as folder:
            ...
            self.assertEqual(dict(records=2), FileIO.read_json(cache.counts_file))

            @logger.phase(cache)
            def _():
                self.fail("Should not be called the second time")
