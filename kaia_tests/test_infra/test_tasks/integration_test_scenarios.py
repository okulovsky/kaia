...
def two_requests_without_abort(self: unittest.TestCase, proc: TaskProcessor):
    ...
    self.assertListEqual(
        ['null', '{"ticks": 4}', 'null', '{"ticks": 2}', 'null', '"Warmed up True"', '0.0', '"Tick 0/4"', '0.25',
         '"Tick 1/4"', '0.5', '"Tick 2/4"', '0.75', '"Tick 3/4"', 'null', '5', 'null', '"Warmed up True"', '0.0',
         '"Tick 0/2"', '0.5', '"Tick 1/2"', 'null', '3'],
        list(df.payload)
    )
    self.assertListEqual(
    ...
    self.assertDictEqual(
        {'id': id1, 'received': True, 'arguments': {'ticks': 4},
         'accepted': True, 'progress': 0.75, 'aborted': False, 'success': True, 'failure': False, 'error': None,
    ...
    self.assertDictEqual(
        {'id': id2, 'received': True, 'arguments': {'ticks': 2},
         'accepted': True, 'progress': 0.5, 'aborted': False, 'success': True, 'failure': False, 'error': None,
...
def two_requests_one_is_aborted(self: unittest.TestCase, proc: TaskProcessor):
    ...
    df = pd.DataFrame(messages)
    df = df.loc[df.tag_0!='is_alive']
    self.assertGreater(df.loc[(df.tag_1 == id1) & (df.tag_0 == 'log')].shape[0], 2)
